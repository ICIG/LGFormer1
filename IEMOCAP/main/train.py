...
for i in range(1,6):
    ...
    # 设置数据集的输出路径
    out_path = r'/home/lijiahao/DWFormer-main/new_database_wavlm_mask_324/'
    # out_path = r'./new_database_hubert_mask_324/'
    # out_path = r'./randoms/'
    # 定义训练和验证数据集的路径
    trainplace = r'new_database_wavlm_mask_324train' + str(i)
    validplace = r'new_database_wavlm_mask_324valid' + str(i)
    # 使用lmdb_dataset创建数据集实例
    train_dataset = lmdb_dataset(out_path,trainplace)
    develop_dataset = lmdb_dataset(out_path,validplace)
    # 为训练和验证数据集创建DataLoader实例
    ...
    print('当前最好结果: | wa: %.3f' %best_wa, '|ua: %.3f' %best_ua)
