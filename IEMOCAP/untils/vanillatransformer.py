...
class vanilla_transformer_block(nn.Module):
    ...
    def forward(self, x, haltingscore):
        ...
        '''
        x = x.permute([1,0,2])#x[324,32,1024]
        residual = x
        x1, attn = self.mha(key = x, value = x, query = x)#attn(32,324,324)
        x = residual + x1
        x = self.ln1(x)
        residual = x
        x2 = self.FFN(x)
        ...
        #haltingscore shape: torch.Size([32, 324])
        #attn_sum shape: torch.Size([32, 324])
        
        haltingscore += attn
        return x, haltingscore, attn
        #x: [batch, t, c] haltingscore: [batch, t] attn: [batch, t, t]